...
startup
{
    ...
    string statusFile = Directory.GetCurrentDirectory() + "\\superhangon-statusfile.txt";
    if ( File.Exists( statusFile ) ) {
        File.Delete( statusFile );
    }
    vars.statusFile = statusFile;
...
init
{
    ...
    vars.watchersToExport = new string[6]
    {
        "frameHealth",
        "engineHealth",
        "brakeHealth",
    ...
    foreach (string watch in vars.watchersToExport) {
        File.AppendAllText(vars.statusFile, watch + "=" + vars.watchers[watch].Current + "\r\n");
    }
}
...
update {
    ...
    // print("originalTotalRaces=" + vars.watchers["originalTotalRaces"].Current);
    // print("vars.racesWonLastSplit=" + vars.racesWonLastSplit);
    // print("vars.originalMode=" + vars.originalMode);
    ...
    foreach (string watch in vars.watchersToExport) {
        if (vars.watchers[watch].Old != vars.watchers[watch].Current) {
            File.AppendAllText(vars.statusFile, watch + "=" + vars.watchers[watch].Current + "\r\n");
        }
    }
...
split {
    if (vars.originalMode) {
        if (vars.watchers["originalRaceSelected"].Current > 4 && vars.watchers["originalRaceSelected"].Old == 0) {
            if (vars.watchers["originalSponsor"].Current == 0 && vars.watchers["originalTotalRaces"].Current == 0) {
                return false;
            }
            if (vars.watchers["racesWon"].Current != vars.racesWonLastSplit) {
                vars.racesWonLastSplit = vars.watchers["racesWon"].Current;
                return true;
            }
        }
    }
