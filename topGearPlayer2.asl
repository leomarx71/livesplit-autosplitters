...
init // Runs when the emulator process is found
{
    ...
    vars.watchers = new MemoryWatcherList {
        new MemoryWatcher<ushort>((IntPtr)memoryOffset + 0x1F28) { Name = "playerTwoFinishTime" }, //0x1F28 to get the timer of the P2
        ...
        new MemoryWatcher<byte>((IntPtr)memoryOffset + 0x1E76) { Name = "playerTwoCurrentLap" },
    };

    Func <bool> isRaceFinished = () => {
        var currentplayerTwoFinishTime = vars.watchers["playerTwoFinishTime"].Current;
        var currentplayerTwoCurrentLap = vars.watchers["playerTwoCurrentLap"].Current;
        return currentplayerTwoFinishTime > 0 &&
            currentplayerTwoFinishTime < 65535 &&
            currentplayerTwoCurrentLap > 1 &&
            currentplayerTwoCurrentLap < 10;
    };
...
gameTime
{
    ...
    if(vars.isRaceFinished() && vars.isInARace){
        currentCents += vars.counterToCents(vars.watchers["playerTwoFinishTime"].Current);
    }
...
split {
    if(vars.isInARace && vars.isRaceFinished()){
        ...
        vars.isInARace = false;
        int cents = vars.counterToCents(vars.watchers["playerTwoFinishTime"].Current);
